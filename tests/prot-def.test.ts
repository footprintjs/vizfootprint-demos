...
import { PROT_VIEWS, PROT_WORDS, RAMA_VIEW, RESIDUES_TABLE, RESIDUE_KEY, SHEET_VIEW, STRUCTURE_VIEW, protDef, protGrains } from '../src/prot/def.js';
...
describe('the def the desk ships', () => {
  it('declares one table, keyed by the minted residue key, and three views', () => {
    ...
    expect(PROT_VIEWS).toEqual([STRUCTURE_VIEW, RAMA_VIEW, SHEET_VIEW]);
  });
  ...
  it('declares the grain where the marks are: one mark per ROW, at all three addresses', () => {
