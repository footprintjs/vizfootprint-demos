 ...
 * turn that orients, selects, analyzes and names a beat. What lands must be
 * agent-badged commits in the one log — and a checkpoint a person can seek to.
 */
describe('the NNDSS analyst (scripted provider)', () => {
  it('drives the dashboard through the verbs and its acts land in the log, badged agent', async () => {
    ...
    expect(analyst.tools).toEqual(['whats_here', 'dispatch', 'declare_analysis', 'why', 'fork', 'checkpoint', 'paths', 'compare', 'propose_chart']);

    const turn = await analyst.send('Focus on pertussis by area and save the moment.');
    expect(turn.text).toMatch(/Pertussis by area/);
    expect(acts.map((a) => a.tool)).toEqual(['whats_here', 'dispatch', 'declare_analysis', 'checkpoint']);
